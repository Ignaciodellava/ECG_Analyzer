...
%Si la frecuencia de muestreo son 360 muestras por segundo, un minuto seran
%21600 muestras
[signal,Fs,tm]=rdsamp('mitdb/119',[],21600,0);
signal=signal(:,1);
...
figure(13),plot(eje,x6), hold on, plot(eje(localizacion),x6(localizacion),'r*')
%Vemos a que posicion corresponden de la señal original
figure(14),plot(eje,signal), hold on, plot(eje(localizacion),signal(localizacion),'r*')

%----------------------------Actividad 5b-----------------------------------
%Rudimentario
%Hacemos una estimacion de la longitud del qrs
finalqrs=localizacion;
principioqrs=finalqrs-60;
%NEGRO=inicio
%AMARILLO=final
figure(15),plot(eje,signal), hold on, plot(eje(finalqrs),signal(finalqrs),'y*'),hold on, plot(eje(principioqrs),signal(principioqrs),'k*')
...
%la base de datos physionet que son los supuestamente acertados
 ann=rdann('mitdb/119','atr',[],21600,0);
...
%cada latido. 
figure(16),plot(eje,signal),hold on, plot(eje(ann),signal(ann), 'g*'),hold on, plot(eje(localizacion),x6(localizacion),'r*');
